...
ros::Publisher pub1("laser_d1", &laser_dist1); // rostopic name for Lasar sensor 1
ros::Publisher pub2("laser_d2", &laser_dist2);
ros::Publisher pub3("laser_d3", &laser_dist3);
ros::Publisher pub4("laser_th", &laser_angle);

const int PIN_ANALOG_INPUT1 = 4; // Define analog pin ID for Lasar sensor 1
...
{
  node.initNode();
  node.advertise(pub1);  // rostopic publisher
  node.advertise(pub2);
  node.advertise(pub3);
  node.advertise(pub4);
}
...
void loop()
{
  int i1 = analogRead( PIN_ANALOG_INPUT1 ); //Analog output voltage of Lasar Sensor 1 
  int i2 = analogRead( PIN_ANALOG_INPUT2 );
  int i3 = analogRead( PIN_ANALOG_INPUT3 );
  float f1 = i1 * 5.0 / 1023.0; //  Adjust voltage to resolution of Arduino Analog Input(0~5V -> 0~1023)
  float f2 = i2 * 5.0 / 1023.0;
  float f3 = i3 * 5.0 / 1023.0;
  float pos1 = -f1* 400/5 + 597; // Convert Voltage[V] into Distance[mm]
  float pos2 = -f2* 400/5 + 597;
  float pos3 = -f3* 400/5 + 597;
  float angle = (atan2((pos2-pos1),30.0))*180/PI; //Calculate angle using difference of 2 lasar sensors
